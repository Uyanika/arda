...
with call:
        cityName = raw_input('Adres veya Meydan Adı giriniz: ')
        curs = call.cursor()
            ...
            for x in drange(0,0.040,0.001): # yaklasik 250 km menzil/ 5 km artıs
                for row in rows:
                    ...
                    if target.buffer(x).intersects(Point(x1, y1)):
                        i = i + 1
                        print  str(i)+' : '+unicode(row[3])+' ('+unicode(row[2])+','+unicode(row[4])+ u') meydanı '+str(distance(origin, target1))+  u' km uzakta'
